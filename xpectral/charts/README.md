...
- **`theme_manager.py`** — `theme.set(name)` to switch the active Bokeh theme. Built-in Bokeh themes (`caliber`, `carbon`, `light_minimal`, `dark_minimal`, `night_sky`, `contrast`) plus a custom `ocean` theme; defaults to `light_minimal`.
